...
#     out_img=np.zeros_like(albu_out)
#     count=0
...
#                 out_img[:,:,count] = tsting_single_cal.testImages(albu_out[:,:,count], dm_op, model)
#                 count=count+1

#     return out_img


def dmpp_cal(width,height, albu_out, dm_opL, mask_image, gpu_device,model):

    
    # model = dmnet('DM++/from-samik/dmnet_membrane_MBA.hdf5')

    out_img = np.zeros_like(albu_out,dtype=np.uint8)
    count = 0
    ...
    for row in range(0, width-511, 512):
        for column in range(0, height-511, 512):
            if np.sum(mask_image[row:row+512, column:column+512]):
                dm_op = toimage(dm_opL[count], cmin=0.0, cmax=1.0)
                dm_op = np.asarray(dm_op)
                # print("shape of dm_op: ",dm_op.shape)
                dm_out[row:row+512,column:column+512]=255*dm_op
                out_img[:, :, count] = tsting_single_cal.testImages(albu_out[:, :, count], dm_op, model)
                count += 1

    # cv2.imwrite('/dm_out.jpg',dm_out)
    return out_img
