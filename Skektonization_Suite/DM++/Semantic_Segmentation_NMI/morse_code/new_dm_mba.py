...
GAUSS = 2
# OUTDIR = '/dev/shm/pratik_scratch_d/temp4'
PERSISTENCE_THRESHOLD = 16
l = 512
w = 512
...
def cmp_dm_img_tri2D(output_dir, input_filename, threshold):
    # print('running morse')
...
def make_png(verts, edges, l, w, linestroke=1):
...
def dm_cal(tile, id, ps, OUTDIR):
    # start = time.time()
    PERSISTENCE_THRESHOLD=ps
    img = tile
    img = np.asarray(img)
    ...
    verts, edges = cmp_dm_img_tri2D(id_dir, input_filename,threshold)
    ...
    above = img[img >= perc]
    if above.size == 0:
        MAX_INT = 8  # Default value if no values are above the threshold
    else:
        MAX_INT = int(np.mean(above)) + 8
    ...
    # with open(id_dir + 'paths.txt', 'r') as file:
    #     for line in file:
    ...
    morse_tile=make_png(verts, output_edges, nx, ny, linestroke=1)
