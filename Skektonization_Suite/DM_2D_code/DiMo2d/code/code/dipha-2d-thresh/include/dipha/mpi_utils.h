...
namespace dipha
{
  namespace mpi_utils
  {
    ...
    {
      std::cout << "entering into file_open" << std::endl;
      MPI_File file;
    ...
    inline MPI_File file_open_read_write(const std::string& filename) {
    std::cout << "Opening file: " << filename << " for read/write." << std::endl;
    
    return file_open(filename, MPI_MODE_RDWR | MPI_MODE_CREATE);
}
  }
}
