...
from albu_calculations_gray_STP import *
...
def imwrite_fast(img_path, opImg):
    ...
    base = base[0:-4]
    img = tiff.imwrite('temp/'+base+'.tif', opImg) # Needs a temp folder for intermediate TIFF image in the CWD
...
def dm_fn(tile,id,temp_dir):
    persistence_th=128
    # print("Persistence Threshold: ",persistence_th)
    # dm_op = new_dm_mba.dm_cal(tile,id,persistence_th)
...
def main(input_image_path,json_out_dir,brain_no,section_num,albu_models,model_dmpp,temp_dir,scratch_dir,json_out_dir_temp):
    ...
    # Getting mask
    # mask_image=mask(img)
    mask_image = np.ones((width,height),dtype='uint8')
    ...
    for row in range(0, width-511, 512):
        for column in range(0, height-511, 512):
            if np.sum(mask_image[row:row+512,column:column+512]):
                tile.append(img[row:row+512,column:column+512])
                id.append(count)
                count = count + 1
    ...
    cv2.imwrite('albu_out.jpg', likelihood_image)
    ...
    background_image=np.zeros((width,height,3),dtype='uint8')
